...
def main(config_path, **kwargs):
    ...
    x_dataset = torch.utils.data.Subset(
        torchvision.datasets.CIFAR10(
            config.dataset_path, train=True, transform=transform_x, download=True
        ),
        x_indices,
    )
    u_dataset = UDataset(
        *[
            torch.utils.data.Subset(
                torchvision.datasets.CIFAR10(
                    config.dataset_path, train=True, transform=transform_u, download=True
                ),
                u_indices,
            )
            for _ in range(2)
...
def build_x_u_split(dataset, num_labeled):
    ...
    u_indices, x_indices = next(
        StratifiedShuffleSplit(n_splits=1, test_size=num_labeled, random_state=42).split(
            targets, targets
        )
    )
...
def mix_up(left, right, a):
    ...
    return [
        weighted_sum(l, r, a=lam.view(lam.size(0), *[1 for _ in range(l.dim() - 1)]))
...
def mix_match(x, u, model, config):
    ...
    # predict u targets ################################################################################################
    images_u = torch.cat([images_u_0, images_u_1])
    targets_u_0, targets_u_1 = (
        model(images_u).detach().split([images_u_0.size(0), images_u_1.size(0)])
    )
    ...
    # shuffle ##########################################################################################################
    images_w = torch.cat([images_x, images_u])
    targets_w = torch.cat([targets_x, targets_u])
    ...
    images_x, targets_x = mix_up(
        left=(images_x, targets_x), right=(images_w_x, targets_w_x), a=config.train.mix_match.alpha
    )
    images_u, targets_u = mix_up(
        left=(images_u, targets_u), right=(images_w_u, targets_w_u), a=config.train.mix_match.alpha
    )
...
def train_epoch(model, data_loader, optimizer, scheduler, epoch, config):
    ...
    for (images_x, targets_x), (images_u_0, images_u_1) in tqdm(
        ...
        # mix-match ####################################################################################################
        with torch.no_grad():
            (images_x, targets_x), (images_u, targets_u) = mix_match(
                x=(images_x, targets_x), u=(images_u_0, images_u_1), model=model, config=config
            )
        ...
        weight_u = config.train.mix_match.weight_u * min((epoch - 1) / config.epochs_warmup, 1.0)
    ...
    with torch.no_grad():
        ...
        writer.add_image(
            "images_x",
            torchvision.utils.make_grid(images_x, nrow=compute_nrow(images_x), normalize=True),
            global_step=epoch,
        )
        writer.add_image(
            "images_u",
            torchvision.utils.make_grid(images_u, nrow=compute_nrow(images_u), normalize=True),
            global_step=epoch,
        )
...
def eval_epoch(model, data_loader, epoch, config):
    ...
    with torch.no_grad():
        model.eval()
        for images_x, targets_x in tqdm(
