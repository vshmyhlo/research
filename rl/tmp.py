...
class TransitionList:
    ...
    def build_batch(self):
        kvs = defaultdict(lambda: [])
        for trns in self.transitions:
            for k, v in trns.items():
                kvs[k].append(v)
        kvs = {k: jnp.stack(v, 0) for k, v in kvs.items()}
        return kvs
...
def build_agent(obs_space, act_space):
    ...
    #     logits = hk.nets.MLP([32, act_space.n])(emb)
    #     v = jnp.squeeze(hk.nets.MLP([32, 1])(emb), -1)
...
def main(run_id):
    ...
    def opt_step(params, opt_state, batch):
        ...
        def compute_loss(params, batch):
            ...
            return loss, (critic_loss, actor_loss, v_target, td_error, v_tm1)
    ...
    for step_i in tqdm(range(1, num_observations + 1)):
        ...
        if step_i % 256 == 0:
            ...
            params, opt_state, (critic_loss, actor_loss, v_target, td_error,
                                v_tm1), = opt_step(params, opt_state, batch)
