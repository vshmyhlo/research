...
class Dataset2020KFold(torch.utils.data.Dataset):
    def __init__(self, path, train, fold, transform=None):
        ...
        folds = SimpleStratifiedGroupKFold(FOLDS, shuffle=True, random_state=42).split(
            data["target"], data["target"], data["patient_id"]
        )
    ...
    def __getitem__(self, i):
        ...
        input = {
            ...
            "meta": {
                ...
                "site": np.array(
                    SITE_TO_ID[sample["anatom_site_general_challenge"]], dtype=np.int64
                ),
            },
    ...
    def load_data(path):
        ...
        data["jpeg_path"] = data["image_name"].apply(
            lambda x: os.path.join(path, "jpeg", "train", "{}.jpg".format(x))
        )
...
class Dataset2020Test(torch.utils.data.Dataset):
    ...
    def load_data(path):
        ...
        data["jpeg_path"] = data["image_name"].apply(
            lambda x: os.path.join(path, "jpeg", "test", "{}.jpg".format(x))
        )
        data = data.reset_index(drop=True)
...
class Dataset2019(torch.utils.data.Dataset):
    ...
    def load_data(path):
        ...
        data["target"] = data["MEL"] == 1.0
        data["jpeg_path"] = data["image"].apply(
            lambda x: os.path.join(
                path, "ISIC_2019_Training_Input", "ISIC_2019_Training_Input", "{}.jpg".format(x)
            )
        )
        data = data.reset_index(drop=True)
