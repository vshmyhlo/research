...
def build_graph(lr, w, tw):
    ...
    teacher_outputs = {
        'class_index': tf.argmax(teacher(inputs['x_s'], psi), -1)
    }
    teacher_outputs['accuracy'] = tf.reduce_mean(tf.cast(tf.equal(
        inputs['y_s'],
    ...
    student_outputs['accuracy'] = tf.reduce_mean(tf.cast(tf.equal(
        inputs['y_s'],
        ...
        for i in tqdm(range(1, 100000 + 1)):
            ...
            if i % 100 == 0:
                ...
                fig = plot_decision_boundary(
                    x_s, y_s, lambda x: sess.run(outs['teacher']['class_index'], feed_dict={ins['x_s']: x}))
                writer.add_figure('teacher/sup', fig, global_step=i)
                fig = plot_decision_boundary(
                    x_val, y_val, lambda x: sess.run(outs['teacher']['class_index'], feed_dict={ins['x_s']: x}))
                writer.add_figure('teacher/val', fig, global_step=i)
                fig = plot_decision_boundary(
                    x_s, y_s, lambda x: sess.run(outs['student']['class_index'], feed_dict={ins['x_s']: x}))
                writer.add_figure('student/sup', fig, global_step=i)
                fig = plot_decision_boundary(
                    x_val, y_val, lambda x: sess.run(outs['student']['class_index'], feed_dict={ins['x_s']: x}))
