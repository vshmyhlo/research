...
class Model(nn.Module):
    def __init__(self, model, vocab_size, sample_rate, mean_std):
        ...
        self.spectra = Spectrogram(
            sample_rate=sample_rate, num_mels=model.num_mels, mean_std=mean_std
        )
...
class PostNet(nn.Sequential):
    def __init__(self, num_mels, mid_features, num_layers=5):
        ...
        for i in range(num_layers - 1):
            blocks.append(
                ConvNorm1d(
                    num_mels if i == 0 else mid_features, mid_features, 5, padding=2, init="tanh"
                )
            )
