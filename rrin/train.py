...
import utils
from rrin.dataset import Vimeo90kDataset, ForEach, RandomTemporalFlip, RandomHorizontalFlip
...
def main(config_path, **kwargs):
    ...
    eval_transform = ForEach(T.Compose([
        T.ToTensor(),
    ...
    train_data_loader = torch.utils.data.DataLoader(
        Vimeo90kDataset(
            config.dataset_path, subset='train', transform=train_transform),
    ...
    eval_data_loader = torch.utils.data.DataLoader(
        Vimeo90kDataset(
            config.dataset_path, subset='test', transform=eval_transform),
        batch_size=config.batch_size,
        shuffle=False,
        num_workers=os.cpu_count(),
        drop_last=False)
...
def denormalize(input):
    ...
    return (input * 0.25 + 0.5).clamp(0., 1.)
