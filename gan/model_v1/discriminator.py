...
from gan.modules import ConvEq, NoOp, ReLU, Upsample
...
class Discriminator(nn.Module):
    def __init__(self, image_size):
        def build_level_layers(level, base_channels=16):
            ...
            if level == 0:
                conv = nn.Sequential(MidBlock(in_channels, in_channels), ZeroBlock(in_channels))
            else:
                conv = nn.Sequential(
...
class ZeroBlock(nn.Sequential):
    def __init__(self, in_channels):
        super().__init__(ConvEq(in_channels, 1, 4))
